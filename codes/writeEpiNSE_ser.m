...
gx = mr.makeTrapezoid('x',lims,'FlatArea',kWidth*2,'FlatTime',readoutTime); %SER
adc = mr.makeAdc(Nx*ro_os*2,lims,'Duration',gx.flatTime,'Delay',gx.riseTime); %SER
...
gzReph2 = mr.makeTrapezoid('z',lims,'Area',-gz.area/2,'Duration',preTime);
...
gxPre = mr.makeTrapezoid('x',lims,'Area',gx.area/4-deltak/2,'Duration',preTime);
gxPre.amplitude = -gxPre.amplitude;
gxPre2 = mr.makeTrapezoid('x',lims,'Area',gx.area/2-deltak/2,'Duration',preTime);
gxPre2.amplitude = -gxPre2.amplitude;
gyPre = mr.makeTrapezoid('y',lims,'Area',Ny/2*deltak,'Duration',preTime);
...
for s=1:Nslices/2 %SER
    ...
    rf.phaseOffset=-2*pi*rf.freqOffset*mr.calcRfCenter(rf); % compensate for the slice-offset induced phase
    seq.addBlock(rf,gz);
    seq.addBlock(gzReph2);

    
    seq.addBlock(gxPre,gyPre);

    seq.addBlock(mr.makeDelay(5e-3));
...
seq.write('epi_nse_ser.seq');   % Output sequence for scanner
