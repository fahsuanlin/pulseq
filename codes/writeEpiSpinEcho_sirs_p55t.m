...
thickness=3e-3;
% Set system limits
lims = mr.opts('MaxGrad', 19, 'GradUnit', 'mT/m',...
    'MaxSlew', 38, 'SlewUnit', 'T/m/s', 'rfRingdownTime', 30e-6, ...
    'rfDeadTime', 100e-6, 'adcDeadTime', 10e-6, 'B0', 0.55);  
...
[rfsl1, gzsl1, gzRephsl1] = mr.makeSincPulse(pi/2,'system',lims,'Duration',2e-3,...
    'SliceThickness',thickness,'apodization',0.42,'timeBwProduct',4,'use','excitation');
% Create spin-lock pulse and gradient
f_sl=15; %Hz, spin-locking frequency
t_sl=1e0; %s, spin-locking time
[rfsl] = mr.makeBlockPulse(2*pi*f_sl,'system',lims,'Duration',t_sl,'phaseOffset', pi/2,...
    'SliceThickness',thickness,'timeBwProduct',4,'use','excitation');
% Create -90 degree slice selection pulse and gradient
[rfsl2, gzsl2, gzRephsl2] = mr.makeSincPulse(-pi/2,'system',lims,'Duration',2e-3,...
    'SliceThickness',thickness,'apodization',0.42,'timeBwProduct',4,'use','excitation');
% gradient spoiling
gxSpoil=mr.makeTrapezoid('x',lims,'Area',2*Nx/fov);
gzSpoil=mr.makeTrapezoid('z',lims,'Area',4/thickness);
...
kWidth = Nx*deltak;
readoutTime = 3.2e-4;
readoutTime = lims.adcRasterTime*100*Nx;
...
seq.write('epi_se_sirs_p55t.seq');   % Output sequence for scanner
seq.plot('stacked',1);             % Plot sequence waveforms
