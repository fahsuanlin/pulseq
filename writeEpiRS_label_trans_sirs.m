...
% Set system limits
sys = mr.opts('MaxGrad',19,'GradUnit','mT/m',...
    'MaxSlew',38,'SlewUnit','T/m/s',...
    'rfRingdownTime', 30e-6, 'rfDeadtime', 100e-6,...
    'adcDeadTime', 10e-6, 'B0', 0.55 ... % this is Siemens' .55T
);

seq=mr.Sequence(sys);      % Create a new sequence object
fov=220e-3; 
%Nx=96; Ny=Nx;  % Define FOV and resolution
Nx=64; Ny=Nx;  % Define FOV and resolution
...
readoutTime = 860e-6 ; % default value; longer in .55 T scanner...
...
[rfsl1, gzsl1, gzRephsl1] = mr.makeSincPulse(pi/2,'system',sys,'Duration',2e-3,...
    'SliceThickness',thickness,'apodization',0.42,'timeBwProduct',4,'use','excitation');
% Create spin-lock pulse and gradient
f_sl=.2; %Hz, spin-locking frequency
t_sl=1e-3; %s, spin-locking time
[rfsl] = mr.makeBlockPulse(2*pi*f_sl,'system',sys,'Duration',t_sl,'phaseOffset', pi/2,...
    'SliceThickness',thickness,'timeBwProduct',4,'use','excitation');
% Create -90 degree slice selection pulse and gradient
[rfsl2, gzsl2, gzRephsl2] = mr.makeSincPulse(-pi/2,'system',sys,'Duration',2e-3,...
    'SliceThickness',thickness,'apodization',0.42,'timeBwProduct',4,'use','excitation');
% gradient spoiling
gxSpoil=mr.makeTrapezoid('x',sys,'Area',2*Nx/fov);
gzSpoil=mr.makeTrapezoid('z',sys,'Area',4/thickness);
...
minTR_1slice = mr.calcDuration(gz_fs) + mr.calcDuration(gz) + mr.calcDuration(gzReph)+...
    ...
    mr.calcDuration(gzsl1) + mr.calcDuration(rfsl) + mr.calcDuration(gzsl2) +mr.calcDuration(gzSpoil) +...
...
TE=0.06;
TE_min = rf.shape_dur/2 + rf.ringdownTime + mr.calcDuration(gzReph)+...
    Nnav*mr.calcDuration(gx) + mr.calcDuration(gyPre) + ...
    Ny_meas/2*mr.calcDuration(gx) - mr.calcDuration(gx)/2;
TEdelay = TE - TE_min ;

actualTR=(minTR_1slice+TRdelay_perSlice)*Nslices;
disp(['actual TR = ', num2str(actualTR*1e3), ' ms', ', actual TE = ', num2str(1000*(TE_min+TEdelay)), ' ms']) ;
...
%for r=1:Nrep
    ...
    %s=1;
        ...
        seq.addBlock(rfsl1, gzsl1) ;
        seq.addBlock(rfsl) ;
        seq.addBlock(rfsl2, gzsl2) ;
