...
readoutTime = round(sys.adcRasterTime*50*Nx*ro_os/1e-6)*1e-6;
...
if(MBFOVshift)
    ...
    dur_sms = ceil(2*sqrt(deltak_sms_max/sys.maxSlew)/10e-6)*10e-6;
    if(dur_sms>blip_dur)
        gy = mr.makeTrapezoid('y',sys,'Area',-deltak,'Duration',dur_sms);

        adc = mr.makeAdc(adcSamples,'Dwell',adcDwell,'Delay',dur_sms/2);
        % realign the ADC with respect to the gradient
        time_to_center=adc.dwell*((adcSamples-1)/2+0.5); % I've been told that Siemens samples in the center of the dwell period
        adc.delay=round((gx.riseTime+gx.flatTime/2-time_to_center)*1e6)*1e-6; % we adjust the delay to align the trajectory with the gradient. We have to aligh the delay to 1us 
        
        gz_sms = mr.makeTrapezoid('z',sys,'Area',deltak_sms,'Duration',dur_sms);
        gz_sms_rep = mr.makeTrapezoid('z',sys,'Area',-deltak_sms_max,'Duration',dur_sms);

        gy_parts = mr.splitGradientAt(gy, dur_sms/2, sys);
        [gy_blipup, gy_blipdown,~]=mr.align('right',gy_parts(1),'left',gy_parts(2),gx);
        gy_blipdownup=mr.addGradients({gy_blipdown, gy_blipup}, sys);

        gz_sms_parts = mr.splitGradientAt(gz_sms, dur_sms/2, sys);
        [gz_sms_blipup, gz_sms_blipdown,~]=mr.align('right',gz_sms_parts(1),'left',gz_sms_parts(2),gx);

        gz_sms_rep_parts = mr.splitGradientAt(gz_sms_rep, dur_sms/2, sys);
        [gz_sms_rep_blipup, gz_sms_rep_blipdown,~]=mr.align('right',gz_sms_rep_parts(1),'left',gz_sms_rep_parts(2),gx);

        gz_sms_blipdownup=mr.addGradients({gz_sms_blipdown, gz_sms_blipup}, sys);    
        gz_sms_blipdownuprep=mr.addGradients({gz_sms_blipdown, gz_sms_rep_blipup}, sys);    
        gz_sms_blipdownrepup=mr.addGradients({gz_sms_rep_blipdown, gz_sms_blipup}, sys); 
    else
        gz_sms = mr.makeTrapezoid('z',sys,'Area',deltak_sms,'Duration',blip_dur);
        gz_sms_rep = mr.makeTrapezoid('z',sys,'Area',-deltak_sms_max,'Duration',blip_dur);

        gz_sms_parts = mr.splitGradientAt(gz_sms, blip_dur/2, sys);
        [gz_sms_blipup, gz_sms_blipdown,~]=mr.align('right',gz_sms_parts(1),'left',gz_sms_parts(2),gx);

        gz_sms_rep_parts = mr.splitGradientAt(gz_sms_rep, blip_dur/2, sys);
        [gz_sms_rep_blipup, gz_sms_rep_blipdown,~]=mr.align('right',gz_sms_rep_parts(1),'left',gz_sms_rep_parts(2),gx);
...
TE_delay=TE_desired-TE;
assert(TE_delay>=0 ) ;
seq.addBlock(TRdelay_perSlice); disp(['actual TR = ', num2str(actualTR*1e3), ' ms', ', actual TE = ', num2str(1000*(TE+TE_delay)), ' ms']) ;
        ...
        for i = 1:Ny_meas
            ...
            if MBFOVshift == 0
                ...
                if i==1 % first phase encoding step
                    ...
                    elseif(mod(i+1,MBFOVshift)==0)
                        seq.addBlock(gx, gy_blipup, gz_sms_rep_blipup, adc, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                        %seq.addBlock(gx, gy_blipup, gz_sms_rep_blipup, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                    else
                        seq.addBlock(gx, gy_blipup, gz_sms_blipup, adc, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                        %seq.addBlock(gx, gy_blipup, gz_sms_blipup, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                    end;                
                elseif i==Ny_meas % last phase encoding step
                    if(mod(i,MBFOVshift)==0)
                        seq.addBlock(gx, gy_blipdown, gz_sms_rep_blipdown, adc, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                        %seq.addBlock(gx, gy_blipdown, gz_sms_rep_blipdown, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                    else
                        seq.addBlock(gx, gy_blipdown, gz_sms_blipdown, adc, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                        %seq.addBlock(gx, gy_blipdown, gz_sms_blipdown, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                    end;
                else % phase encoding steps in between
                    if(mod(i,MBFOVshift)==0)
                        seq.addBlock(gx, gy_blipdownup, gz_sms_blipdownrepup, adc, lrev, lseg, llin) ; % Read an intermediate line of k-space with a half-blip at the beginning and a half-blip at the end
                        %seq.addBlock(gx, gy_blipdownup, gz_sms_blipdownrepup, lrev, lseg, llin) ; % Read an intermediate line of k-space with a half-blip at the beginning and a half-blip at the end
                    elseif(mod(i+1,MBFOVshift)==0)
                        seq.addBlock(gx, gy_blipdownup, gz_sms_blipdownuprep, adc, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                        %seq.addBlock(gx, gy_blipdownup, gz_sms_blipdownuprep, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                    else
                        seq.addBlock(gx, gy_blipdownup, gz_sms_blipdownup, adc, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                        %seq.addBlock(gx, gy_blipdownup, gz_sms_blipdownup, lrev, lseg, llin) ; % Read the last line of k-space with a single half-blip at the beginning
                    end;
                end
            end;
    	    gx = mr.scaleGrad(gx, -1) ;   % Reverse polarity of read gradient            
